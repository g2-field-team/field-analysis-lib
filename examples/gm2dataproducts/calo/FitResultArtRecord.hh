...
namespace gm2calo {
	...
	FitResultArtRecord() :
	    amcHeader(), riderChannelHeader(), fillNum(0), caloNum(0), islandNum(0), xtalNum(0), riderNum(0), 
	...
	FitResultArtRecord(int f_n, int c_n, int i_n, int x_n, int r_n,  int rc_n, 
	    ...
	    amcHeader(), riderChannelHeader(), fillNum(f_n), caloNum(c_n), islandNum(i_n), xtalNum(x_n), riderNum(r_n),
	    riderChannelNum(rc_n), utcaSlotNum(u_n), time(t), energy(ene), pedestal(ped), pulsewidth(pw), chi2(cnd)
	{}
#endif // __GCCXML__
